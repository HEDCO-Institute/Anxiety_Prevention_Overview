...
num_elig_stud <- sum(study_elig_df$study_eligibility_decision == "Eligible")
...
num_irct <- sum(study_td$study_study_research_design == "Randomized trial" & study_td$study_assignment_level == "Individual")
...
The `r num_elig_stud` included primary studies included `r num_studs` students (median: `r median(study_td$study_number_participants, na.rm = TRUE)` students per study, range: `r min(study_td$study_number_participants, na.rm = TRUE)` to `r format_thousands(max(study_td$study_number_participants, na.rm = TRUE))`), `r sum(study_td$study_number_classrooms, na.rm = TRUE)` classrooms (median: `r median(study_td$study_number_classrooms, na.rm = TRUE)` classrooms per study, range: `r min(study_td$study_number_classrooms, na.rm = TRUE)` to `r max(study_td$study_number_classrooms, na.rm = TRUE)`, and `r sum(study_td$study_number_schools, na.rm = TRUE) ` schools (median: `r median(study_td$study_number_schools, na.rm = TRUE)` schools per study, range: `r min(study_td$number_schools, na.rm = TRUE)` to `r max(study_td$number_schools, na.rm = TRUE)`). The average age of students ranged from `r round(min(study_td$study_age_average, na.rm = TRUE), 1)` to `r max(study_td$study_age_average, na.rm = TRUE)` (with a median of `r round(median(study_td$study_age_average, na.rm = TRUE), 1)`). Regarding school level, `r study_elem` studies (`r format_percent(study_elem_per, 1)`) took place in primary/elementary school, `r study_mid` studies (`r format_percent(study_mid_per, 1)`) took place in intermediate/middle school, and `r study_high` studies (`r format_percent(study_high_per, 1)`) took place in secondary/high school. The most common grade levels were 1st grade (`r count_grade_level$n[1]` studies; `r format_percent(count_grade_level$percent[1], 1)`), 9th grade (`r count_grade_level$n[2]` studies; `r format_percent(count_grade_level$percent[2], 1)`), 7th grade (`r count_grade_level$n[3]` studies; `r format_percent(count_grade_level$percent[3], 1)`), 8th grade (`r count_grade_level$n[4]` studies; `r format_percent(count_grade_level$percent[4], 1)`), and 10th grade (`r count_grade_level$n[5]` studies; `r format_percent(count_grade_level$percent[5], 1)`). The average percentage of students who were female ranged from `r min_female` to `r max_female`(with a median of `r med_female`). Of the `r sum(study_td$country == "United States")` studies conducted in the United States, the average percentage of students who were white ranged from `r min_white` to `r max_white` (with a median of `r med_white`). *No studies conducted in the United States reported the percentage of English language learners. Two studies reported the percentage of students receiving free or reduced price lunch: Pössel et al. (2013) had `possel_frpl`and McLaughlin (2010) had `mclaughlin_frpl`.*
...
count_delivery <- grp_int %>% 
  filter(study_group_frequency  != "-999") %>% 
  group_by(study_group_frequency ) %>% 
  summarize(n = n(),
            percent = n/sum(grp_int$study_group_frequency != "-999")*100)
...
count_intloc <- grp_int %>% 
  ...
  mutate(inschl_count  = str_count(study_group_location, "During School Time"),
  ...
  group_by(location) %>% 
  summarize(n = sum(value),
            percent = sum(value)/sum(grp_int$study_group_location != "7. Cannot tell") *100) %>% 
  arrange(desc(n))
...
count_intmode <- grp_int %>% 
  ...
  group_by(mode) %>% 
  summarize(n = sum(value),
            percent = sum(value)/sum(grp_int$study_group_mode != "-999") *100) %>% 
  arrange(desc(n))
...
group_study_flag <- grp_int %>% 
  group_by(refid) %>% 
  mutate(monitor_flag = case_when(any(study_implementation_monitoring == "Yes") ~ "Yes",
                                  TRUE ~ study_implementation_monitoring),
         problem_flag = case_when(any(study_implementation_problems == "Yes") ~ "Yes",
...
Across all `r num_elig_stud` primary studies, `r nrow(grp_int)` experimental interventions were evaluated. Regarding format of the `r nrow(grp_int)` experimental interventions, `r count_format$n[1]` (`r format_percent(count_format$percent[1], 1)`) were administered to individual students, `r count_format$n[2]` (`r format_percent(count_format$percent[2], 1)`) were administered to small groups, `r count_format$n[3]` (`r format_percent(count_format$percent[3], 1)`) were administered to whole classes, and `r count_format$n[4]` (`r format_percent(count_format$percent[4], 1)`) was administered to whole schools. The duration of interventions ranged from `r min(grp_int$intervention_weeks, na.rm = TRUE) ` to `r max(grp_int$intervention_weeks, na.rm = TRUE) ` weeks (with a median of `r median(grp_int$intervention_weeks, na.rm = TRUE)`), the number of sessions ranged from `r min(grp_int$intervention_sessions, na.rm = TRUE)` to `r max(grp_int$intervention_sessions, na.rm = TRUE)` sessions (with a median of `r median(grp_int$intervention_sessions, na.rm = TRUE)`), and the length of each session ranged from `r min(grp_int$intervention_intensity, na.rm = TRUE)` to `r max(grp_int$intervention_intensity, na.rm = TRUE)` minutes (with a median of `r median(grp_int$intervention_intensity, na.rm = TRUE)`). Of the `r sum(grp_int$intervention_frequency != "-999")` interventions for which frequency of intervention delivery was reported, `r count_delivery$n[1]` interventions (`r format_percent(count_delivery$percent[1], 1)`) were delivered less than weekly, `r count_delivery$n[2]` interventions (`r format_percent(count_delivery$percent[2], 1)`) were delivered once a week, `r count_delivery$n[3]` interventions (`r format_percent(count_delivery$percent[3], 1)`) were delivered 2-4 times a week, and `r count_delivery$n[4]` intervention (`r format_percent(count_delivery$percent[4], 1)`) involved daily contact. Of the `r sum(grp_int$intervention_provider != "8. Cannot Tell")` interventions for which provider information was reported, `r count_intprovider$n[1]` interventions (`r format_percent(count_intprovider$percent[1], 1)`) by behavioral health personnel, `r count_intprovider$n[2]` interventions (37.0%`r format_percent(count_intprovider$percent[2], 1, force_decimal = TRUE)`) by researchers, `r count_intprovider$n[3]` interventions (`r format_percent(count_intprovider$percent[3], 1)`) were delivered by teachers, `r count_intprovider$n[5]` (`r format_percent(count_intprovider$percent[5], 1)`) were self-administered, `r count_intprovider$n[7]` interventions (`r format_percent(count_intprovider$percent[7], 1)`) by guidance counselors, `r count_intprovider$n[6]` interventions (`r format_percent(count_intprovider$percent[6], 1)`) by other school personnel, and `r count_intprovider$n[4]` interventions (`r format_percent(count_intprovider$percent[4], 1)`) by other providers. Of the `r sum(grp_int$intervention_recipients != "7. Cannot Tell")` interventions for which recipient information was reported, all interventions (`r format_percent(count_intrecip$percent[1], 1)`) were delivered directly to students, `r count_intrecip$n[2]` interventions (`r format_percent(count_intrecip$percent[2], 1)`) to family, `r count_intrecip$n[3]` intervention (`r format_percent(count_intrecip$percent[3], 1)`) to the community, `r count_intrecip$n[4]` intervention (`r format_percent(count_intrecip$percent[4], 1)`) to peers, `r count_intrecip$n[5]` intervention (`r format_percent(count_intrecip$percent[5], 1)`) to staff, and `r count_intrecip$n[6]` intervention (`r format_percent(count_intrecip$percent[6], 1)`) to teachers. In addition to delivery during school hours, `r count_intloc$n[2]` interventions (`r format_percent(count_intloc$percent[2], 1)`) also involved a component that took place out-of-school time but on school groups (e.g., after school), `r count_intloc$n[3]` interventions (`r format_percent(count_intloc$percent[3], 1)`) involved a component that took place at home, and `r count_intloc$n[4]` intervention (`r format_percent(count_intloc$percent[4], 1)`) involved a component that took place in the community. Of the `r sum(grp_int$intervention_mode != "-999")` interventions for which intervention mode was reported, `r count_intmode$n[1]` interventions (`r format_percent(count_intmode$percent[1], 1, force_decimal = TRUE)`) involved in-person delivery, `r count_intmode$n[2]` interventions (`r format_percent(count_intmode$percent[2], 1)`) involved digital delivery, and no interventions (`r format_percent(count_intmode$percent[3], 1)`) involved phone delivery. Across the `r num_elig_stud` primary studies, `r num_implem_mon` (`r format_percent(num_implem_mon_per, 1, force_decimal = TRUE)`) reported monitoring intervention implementation to assess delivery as intended, and `r num_implem_prob` (`r format_percent(num_implem_prob_per, 1)`) reported that there was uncontrolled variation or degradation in intervention implementation.
...
Overall, the `r num_elig_stud` primary studies had `r nrow(grp_comp)` comparison interventions. Of these `r nrow(grp_comp)` comparison interventions, `r count_comptype$n[1]` (`r format_percent(count_comptype$percent[1], 1)`) involved business-as-usual, `r count_comptype$n[2]` (`r format_percent(count_comptype$percent[2], 1)`) involved no intervention, `r count_comptype$n[3]` (`r format_percent(count_comptype$percent[3], 1, force_decimal = TRUE)`) involved a wait-list control, `r count_comptype$n[4]` (`r format_percent(count_comptype$percent[4], 1)`) involved a different active intervention, and `r count_comptype$n[5]` (`r format_percent(count_comptype$percent[5], 1)`) involved an attention-control intervention. 
...
Of the `r nrow(irob_td)` individually-randomized trials, we rated `r count_irobtot$n[1]` studies (`r format_percent(count_irobtot$percent[1], 1)`) as high risk of bias and `r count_irobtot$n[2]` studies (`r format_percent(count_irobtot$percent[2], 1)`) as having some concerns. Concerns about risk of bias were most commonly related to missing outcome data (`r count_irobratings$n[1]` studies; `r format_percent(count_irobratings$percent[1], 1)`), random allocation (`r count_irobratings$n[2]` studies; `r format_percent(count_irobratings$percent[2], 1)`), selective reporting (`r count_irobratings$n[3]` studies; `r format_percent(count_irobratings$percent[3], 1)`), and assignment/adherence deviation (`r count_irobratings$n[4]` studies; `r format_percent(count_irobratings$percent[4], 1)`). Of the `r nrow(crob_td)` cluster-randomized trials, we rated `r count_crobtot$n[1]` studies (`r format_percent(count_crobtot$percent[1], 1)`) as high risk of bias and `r count_crobtot$n[2]` studies (`r format_percent(count_crobtot$percent[2], 1)`) as having some concerns. Concerns about risk of bias were most commonly related to missing outcome data (`r count_crobratings$n[1]` studies; `r format_percent(count_crobratings$percent[1], 1)`), assignment/adherence deviation (`r count_crobratings$n[2]` studies; `r format_percent(count_crobratings$percent[2], 1)`), measurement (`r count_crobratings$n[3]` studies; `r format_percent(count_crobratings$percent[3], 1)`), and recruitment (`r count_crobratings$n[4]`studies; `r format_percent(count_crobratings$percent[4], 1)`). Of the `r nrow(robins_td)` non-randomized trials, we rated `r count_robinstot$n[1]` studies (`r format_percent(count_robinstot$percent[1], 1)`) as serious risk of bias and `r count_robinstot$n[2]` studies (`r format_percent(count_robinstot$percent[2], 1)`) as having moderate risk of bias. Serious or critical concerns about risk of bias were most commonly related to confounding (`r count_robinsratings$n[1]` studies; `r format_percent(count_robinsratings$percent[1], 1)`).

# Translational Product

Information for initial translational product

*STUDENT N FOR MA OUTCOMES*
...
# ES reconcilied for Maria

```{r}
# es_raw <- import(here("data", "APO_effect_level_only.csv"))
# 
# es_td <- es_raw %>% 
#   janitor::clean_names() %>% 
#   distinct(across(-user), .keep_all = TRUE)
